...
class AggressiveNearestAgent(NearestPlanetAgent):
    ...
    def act(self, obs, config=None, **kwargs):
        ...
        for tp in sorted_targets:
            if tp.id in self.awaiting_results:
                if self.awaiting_results[tp.id][0] > 0:
                    self.awaiting_results[tp.id][0] -= 1
                    continue
            ...
            if tp.owner != -1:
                extra_ships = 6 * tp.production

            else:
                extra_ships = 0

            if available <= tp.ships + extra_ships + 1:
                continue
        ...
        # TODO: After a while, the agent will have quite a big territory,
        #       That's when it should start moving ships from back planets to the front planets
